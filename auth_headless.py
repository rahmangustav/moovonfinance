"""Login OAuth YouTube tanpa browser di mesin ini (headless), 2 langkah.

run_console() sudah dihapus & flow OOB dimatikan Google, jadi pakai loopback
redirect + salin kode manual.

LANGKAH 1 (tanpa argumen) — cetak URL otorisasi:
    .venv/bin/python auth_headless.py

  Buka URL itu di browser HP/laptop lain -> login akun Moovon -> setujui.
...
LANGKAH 2 (dengan kode/URL sebagai argumen) — tukar jadi token.json:
    .venv/bin/python auth_headless.py "PASTE_CODE_ATAU_URL_DISINI"
...
CLIENT_SECRETS = ROOT / "config" / "client_secrets.json"
TOKEN_FILE = ROOT / "config" / "token.json"
PENDING = ROOT / "config" / ".oauth_pending.json"
...
def step1_print_url():
    ...
    print("\n=== LANGKAH 1: buka URL ini di browser HP/laptop lain ===\n")
    print(auth_url)
    print(
        "\nSetelah setujui, browser diarahkan ke http://localhost/?...code=XXXX "
        "(gagal dimuat = normal).\nSalin kode/URL-nya, lalu jalankan:\n"
        '   .venv/bin/python auth_headless.py "PASTE_DISINI"\n'
    )
